...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        for i in range(len(subblocks)):
            block = subblocks[i]
        #    print(f"BLOCK: {block}")
            subbed = self.compute_s_box(block, i)
        #    print(f"SUB: {subbed}")
            result.extend(subbed)

        if (len(result) != 32): print("UNEXPEXTED LENGTH IN substitute()")
    ...
    def compute_s_box(self,block,round):
        ...
        inner_bits = [block[i] for i in range(1, 5)]
        #print(f"Outer bits: {outer_bits}")
        #print(f"Inner bits: {inner_bits}")
        outer_val = int(''.join(str(x) for x in outer_bits), 2)
        inner_val = int(''.join(str(x) for x in inner_bits), 2)
        #print(f"Outer val: {outer_val}")
        #print(f"Inner val: {inner_val}")
        s_val = S_BOX[round][outer_val][inner_val]
        #print(f"s-val: {s_val}")
        #bin="{0:b}".format(s_val)
        bin = binvalue(s_val, 4)
        return bin
